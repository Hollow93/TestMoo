...
<<<<<<< HEAD
Create ```1.env``` to specify local details, e.g.,
=======
...
<<<<<<< HEAD
For the first time put 1.env file on the host and add GIT_BRANCH=master, specifying which git branch should be deployed on the host.
=======
